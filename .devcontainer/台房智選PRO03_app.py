...
TAIWAN_DATA = {
    ...
    '新北市': {'板橋區': 75, '三重區': 62, '中和區': 65, '永和區': 70, '新莊區': 60, '新店區': 72, '樹林區': 45, '鶯歌區': 38, '三峽區': 48, '淡水區': 35, '汐止區': 55, '土城區': 58, '林口區': 55, '蘆洲區': 55, '五股區': 42, '泰山區': 45},
    '桃園市': {'桃園區': 42, '中壢區': 45, '大溪區': 28, '楊梅區': 25, '蘆竹區': 42, '大園區': 32, '龜山區': 48, '八德區': 35, '龍潭區': 30, '平鎮區': 32},
    '台中市': {'中區': 35, '東區': 40, '南區': 42, '西區': 50, '北區': 45, '西屯區': 65, '南屯區': 58, '北屯區': 55, '豐原區': 35, '大里區': 35, '太平區': 32, '烏日區': 38},
    '台南市': {'中西區': 42, '東區': 45, '南區': 32, '安平區': 42, '安南區': 35, '永康區': 38, '歸仁區': 35, '善化區': 38, '新市區': 35},
    '高雄市': {'新興區': 38, '前金區': 40, '苓雅區': 38, '前鎮區': 42, '鼓山區': 55, '左營區': 48, '楠梓區': 38, '三民區': 35, '鳳山區': 35, '岡山區': 32},
...
DEFAULT_WEIGHTS = {"mrt": 5, "medical": 4, "brand": 4, "school": 3, "park": 3}
DEFAULT_DEDUCTIONS = {"funeral": 5, "cemetery": 5, "tower": 4, "gas": 3, "baseStation": 2}
...
def generate_listings(city: str, district: str, budget_in_ten_thousand: float, weights: dict, deductions: dict):
    ...
    while len(verified_items) < 15 and safety < 500:
        ...
        if nimby_choice:
            multiplier = 18 if nimby_choice["severity"] == "High" else (12 if nimby_choice["severity"] == "Medium" else 6)
            score -= deductions[nimby_choice["key"]] * multiplier

        verified_items.append({
            "id": 1000000 + math.floor(random.random() * 9000000),
            "title": f"{district}{random.choice(TITLE_ADJ)}{random.choice(TITLE_TYPE)}",
            "price": price,
            "pings": pings,
            "age": age,
...
st.markdown(
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;margin-bottom:18px;">
        <div>
            <div style="font-size:34px;font-weight:900;color:white;line-height:1.05;">台房智選 <span style="color:#60a5fa;">PRO 03</span></div>
            <div style="font-size:11px;color:#64748b;font-weight:800;letter-spacing:0.28em;text-transform:uppercase;margin-top:6px;">Taiwan Real Estate Analyzer v7.7</div>
        </div>
...
with st.sidebar:
    ...
    city_options = list(TAIWAN_DATA.keys())
...
st.markdown(
    ...
    <div style="margin-top:22px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
        <div>
            ...
            <div style="font-size:12px;color:#94a3b8;font-weight:700;">已排除 <span style="color:#fb7185;">{st.session_state.purged_count}</span> 件內容偵測為「成交、售出、下架」之物件</div>
        </div>
        ...
        <div class="listing-card">
                ...
                <div style="min-width:220px;">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;">
                        <div style="flex:1;height:10px;background:#1e293b;border-radius:999px;overflow:hidden;">
                            <div style="width:{listing['finalScore']}%;height:100%;background:#10b981;"></div>
                        </div>
                        <div style="color:white;font-size:26px;font-weight:900;">{listing['finalScore']}%</div>
                    </div>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:14px;margin-top:18px;">
                ...
                <div class="diag-box">
                    ...
                    該物件經由 AI 文字比對確認無「成交、售出、不存在」字樣。室內淨空間約 <b>{listing['indoorPings']}</b> 坪，坪效表現極佳，建議進一步人工複核。
                </div>
            </div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:12px;flex-wrap:wrap;margin-top:18px;padding-top:16px;border-top:1px solid rgba(255,255,255,0.08);">
                ...
                <div class="muted">內容有效性校驗：SUCCESS ｜ Sync: {listing['verifiedAt']} ｜ PRO ENGINE v7.7.0</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
