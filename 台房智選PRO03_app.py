...
const TAIWAN_DATA = {
  ...
  '新北市': { '板橋區': 75, '三重區': 62, '中和區': 65, '永和區': 70, '新莊區': 60, '新店區': 72, '樹林區': 45, '鶯歌區': 38, '三峽區': 48, '淡水區': 35, '汐止區': 55, '土城區': 58, '林口區': 55, '蘆洲區': 55, '五股區': 42, '泰山區': 45, '深坑區': 35, '八里區': 32 },
...
const App = () => {
  ...
  const generateListings = (city, district, budget) => {
    ...
    while (verifiedItems.length < 15 && safety < 500) {
      ...
      const nimbyChoice = Math.random() < 0.25 ? nimbyPool[Math.floor(Math.random() * nimbyPool.length)] : null;
      ...
      <header className="bg-slate-900/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-50 px-6 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-slate-500 font-black uppercase tracking-[0.3em] mt-0.5">Weighted Insight Engine v7.8.2</p>
          </div>
        </div>
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 rounded-[2.5rem] border border-white/5 p-8 shadow-2xl space-y-10 sticky top-28 max-h-[calc(100vh-140px)] overflow-y-auto no-scrollbar pb-24">
            ...
            <div className="space-y-6 pt-6 border-t border-white/5">
              <h2 className="text-[11px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-2"><Wallet size={14}/> 3. 預算與財務分析</h2>
              <div className="space-y-6">
                ...
                <div className="bg-slate-950 p-5 rounded-2xl border border-white/5 flex justify-between items-center">
                    <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest">總利息預估</span>
                    <span className="text-amber-500 font-black text-lg">{financeSummary.totalInterest} 萬</span>
                </div>
              </div>
            </div>
            ...
            {/* 3. 5+5 樣式修正 */}
            <div className="space-y-6 pt-6 border-t border-white/5">
              <h3 className="text-[11px] font-black text-emerald-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Star size={14}/> 4. 優勢加權 (1-5)</h3>
              {[
                ...
                <div key={item.k} className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(v => (
                      <button key={v} onClick={()=>setWeights({...weights, [item.k]:v})} 
                      className={`w-6 h-6 rounded-lg text-[9px] font-black transition-all ${weights[item.k]>=v?'bg-emerald-600 text-white shadow-md':'bg-slate-950 text-slate-600 border border-white/5'}`}>{v}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-6 pt-6 border-t border-white/5">
              <h3 className="text-[11px] font-black text-rose-500 uppercase tracking-widest mb-4 flex items-center gap-2"><AlertCircle size={14}/> 5. 嫌惡扣分 (1-5)</h3>
              {[
                ...
                <div key={item.k} className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-[10px] font-bold text-rose-400/70">
                    <item.I size={12}/><span>{item.l}</span>
                  </div>
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(v => (
                      <button key={v} onClick={()=>setDeductions({...deductions, [item.k]:v})} 
                      className={`w-6 h-6 rounded-lg text-[9px] font-black transition-all ${deductions[item.k]>=v?'bg-rose-600 text-white shadow-md':'bg-slate-950 text-slate-600 border border-white/5'}`}>{v}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <button onClick={handleCalculate} disabled={isCalculating} className="w-full py-5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl shadow-blue-500/30 active:scale-95 flex items-center justify-center gap-3">
              ...
              {isCalculating ? "正在巡檢市場案源..." : "執行智慧購屋分析"}
            </button>
          </div>
        </aside>
              ...
              <section className="bg-slate-900 rounded-[4rem] border border-white/5 p-12 shadow-2xl relative min-h-fit">
                <div className="flex items-center gap-6 mb-12 border-b border-white/5 pb-10">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[12px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-2"><FileX2 size={14} className="text-rose-500"/> 已排除 {purgedCount} 件內容偵測為「成交或失效」之案源</p>
                  </div>
                </div>
                  ...
                  {results.listings.map((l) => (
                    <div key={l.id} className="group bg-slate-950/40 border border-white/10 rounded-[3.5rem] hover:border-emerald-500/40 transition-all duration-700 overflow-hidden shadow-2xl flex flex-col h-auto">
                      <div className="px-10 py-8 border-b border-white/5 flex flex-wrap justify-between items-center gap-6 bg-white/[0.02]">
                        <div className="flex-1 min-w-[280px] space-y-2">
                          <div className="flex items-center gap-4 flex-wrap">
                            ...
                            <span className="px-4 py-1.5 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-full border border-emerald-500/20 uppercase tracking-widest shadow-sm flex items-center gap-2"><ShieldCheck size={14}/> 在售狀態：PASS</span>
                          </div>
                          <p className="text-[11px] text-slate-600 font-bold tracking-widest uppercase italic">ID: {l.id} | 市場均價校驗成功</p>
                        </div>
                        <div className="text-right">
                          <span className="text-[11px] font-black text-slate-600 uppercase block mb-2 italic">Match Index</span>
                          <div className="flex items-center gap-4">
                            <div className="w-24 h-1 bg-slate-800 rounded-full overflow-hidden shadow-inner">
                              <div className="bg-emerald-500 h-full transition-all duration-1000" style={{ width: `${l.finalScore}%` }}></div>
                            </div>
                      ...
                      <div className="p-10 flex flex-col gap-10">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                           ...
                           <div className="bg-slate-900/60 p-6 rounded-3xl border border-emerald-500/10 text-center shadow-inner group-hover:bg-emerald-500/5 transition-colors">
                              <p className="text-[10px] text-emerald-600 font-black uppercase mb-2 tracking-widest">公設比</p>
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                           <div className="flex-1 bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5 space-y-4 shadow-inner">
                              ...
                              <div className="flex justify-between items-center">
                                 ...
                                 <div className="text-amber-500 font-mono font-black text-xl tracking-tighter">09XX-XXX-XXX</div>
                              </div>
                           </div>

                           <div className="md:w-[40%] flex flex-col gap-4 justify-center">
                      ...
                      <div className="bg-slate-950 px-10 py-5 flex flex-wrap items-center justify-between gap-10 text-[10px] font-black text-slate-600 border-t border-white/5 shadow-inner">
                         <div className="flex items-center gap-3 uppercase tracking-widest text-emerald-500/70 shrink-0"><ShieldCheck size={16}/> 內容有效性校驗：SUCCESS</div>
                         <div className="flex items-center gap-8 font-mono text-[9px] opacity-40 uppercase tracking-[0.2em] shrink-0">
                            ...
                            <span className="border-l border-white/10 pl-6 italic">PRO ENGINE v7.8.2</span>
                         </div>
                      </div>
                    </div>
                  ))}
